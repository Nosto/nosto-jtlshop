...
switch ($oPlugin->nCalledHook)
{
	...
	case HOOK_SMARTY_OUTPUTFILTER:  //Hook 140
	{
		...
		$smarty->assign('agws_nosto_track_accountname',$oPlugin->oPluginEinstellungAssoc_arr['agws_nosto_track_accountname']);
		...
		if ( isset($_SESSION['agws_nosto_track_orderflag']) && $_SESSION['agws_nosto_track_orderflag'] > 0 )
		{
			...
			for ($i=0;$i<count($bestellung->Positionen);$i++)
			{
				if ($bestellung->Positionen[$i]->nPosTyp==C_WARENKORBPOS_TYP_ARTIKEL && $bestellung->Positionen[$i]->kArtikel>0)
				{
                    ...
                    if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 0)
                        $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $bestellung->Positionen[$i]->Artikel->Preise->fVK[0]));

                    if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 1)
                        $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $bestellung->Positionen[$i]->Artikel->Preise->fVK[1]));
		...
		if (isset($_SESSION['Warenkorb']->PositionenArr) && count($_SESSION['Warenkorb']->PositionenArr) > 0)
		{
			...
			for ($i=0;$i<count($_SESSION['Warenkorb']->PositionenArr);$i++)
			{
				if ($_SESSION['Warenkorb']->PositionenArr[$i]->nPosTyp==C_WARENKORBPOS_TYP_ARTIKEL && $_SESSION['Warenkorb']->PositionenArr[$i]->kArtikel>0)
				{
                    ...
                    if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 0)
                        $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $_SESSION['Warenkorb']->PositionenArr[$i]->Artikel->Preise->fVK[0]));

                    if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 1)
                        $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $_SESSION['Warenkorb']->PositionenArr[$i]->Artikel->Preise->fVK[1]));

                    if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 1)
                        $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $_SESSION['Warenkorb']->PositionenArr[$i]->Artikel->Preise->fVKNetto));
		...
		switch (gibSeitenTyp())
		{
			case PAGE_ARTIKEL:
			{
                ...
                if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 0)
                    $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $agws_nosto_track_product->Preise->fVK[0]));

                if($oPlugin->oPluginEinstellungAssoc_arr[agws_nosto_track_price] == 1)
                    $smarty->assign('agws_nosto_track_PreisNettoBrutto',sprintf("%01.2f", $agws_nosto_track_product->Preise->fVK[1]));
